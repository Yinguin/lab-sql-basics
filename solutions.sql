...
SELECT district_id, COUNT(*) AS number_of_clients FROM client
...
SELECT type, COUNT(*) AS number_of_cards FROM card
GROUP BY type
...
SELECT date, COUNT(DISTINCT loan_id) AS number_of_loans FROM loan
WHERE date < 930907
GROUP BY date
ORDER BY date DESC;
...
SELECT date, duration, COUNT(*) AS number_of_loans FROM loan
...
SELECT account_id, FLOOR(SUM(CASE type WHEN 'PRIJEM' THEN amount WHEN 'VYDAJ' THEN -amount END)) AS difference
FROM trans
GROUP BY account_id
